...
class FriendRequest(models.Model):
    ...
    def clean(self):
        ...
        if BlockedUsers.objects.filter(blocker=self.sender,
                                       blocked=self.receiver).exists():
            raise ValidationError("You blocked this user already")
        if BlockedUsers.objects.filter(blocker=self.receiver,
...
class BlockedUsers(models.Model):
    blocker = models.ForeignKey(get_user_model(), on_delete=models.CASCADE, related_name="blocker")
    blocked = models.ForeignKey(get_user_model(), on_delete=models.CASCADE, related_name="blocked")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
