    ...
    def save(self, commit=True):
        ...
        if commit:
            friendship = Friendship(
                first_user=self.cleaned_data.get("sender"),
...
class RemoveFriendshipForm(forms.Form):
    ...
    def clean(self):
        ...
        friendship = Friendship.objects.filter(
            Q(
                first_user=self.cleaned_data.get("first_user"),
                second_user=self.cleaned_data.get("second_user"),
            )
            | Q(
                first_user=self.cleaned_data.get("second_user"),
                second_user=self.cleaned_data.get("first_user"),
            )
        )
...
class BlockUserForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        Friendship.objects.filter(
            Q(
                first_user=self.cleaned_data.get("blocker"),
                second_user=self.cleaned_data.get("blocked"),
            )
            | Q(
                first_user=self.cleaned_data.get("blocked"),
                second_user=self.cleaned_data.get("blocker"),
            )
        ).delete()
        FriendRequest.objects.filter(
            Q(
                sender=self.cleaned_data.get("blocker"),
                receiver=self.cleaned_data.get("blocked"),
            )
            | Q(
                sender=self.cleaned_data.get("blocked"),
